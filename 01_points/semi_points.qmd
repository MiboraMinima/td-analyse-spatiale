...
1. Récupérer la donnée avec `rnaturalearth`.
    - (bis) Dans le cas où `rnaturalearth` ne fonctionnerait pas, vous pouvez
    télécharger la donnée
    [ici](https://github.com/MiboraMinima/td-analyse-spatiale/raw/refs/heads/main/01_points/data/src/states.gpkg).
    Pour lire une donnée spatiale on utilise le package `sf` avec la fonction
    `read_sf()` qui s'utilise d'une façon similaire à `readr::read_csv()`.
2. Convertissez la donnée en NAD83 (EPSG:5070).

Pour convertir un objet `sf` dans un autre système de projection, on utilise la
fonction `sf::st_transform()`.
...
::: {.callout-tip title="Les packages pour l'analyse centrographique"
collapse=true}

Je vous demande d'écrire les formules vous même pour vous entraîner. Sachez
qu'il existe également des packages qui intègrent des fonctions pour ce genre
d'analyse :

- [`aspace`](https://cran.r-project.org/web/packages/aspace/index.html)
    - **Fonctions :** nombreuses fonctions, on peut réaliser l'ensemble de ce
    qu'il est possible de faire en analyse centrographique.
    - **Points forts :** bien pour contrôler dans le détail l'analyse.
    - **Points faible :** moins *user-friendly* que `sfcentral`, pas intégré à
    `sf`.

- [`sfcentral`](https://gavg712.gitlab.io/sfcentral/) :
    - **Fonctions :** indicateurs de centralité, distance standard (boîte (2D, 3D),
    cercle), ellipse de distribution directionnelle.
    - **Points forts :** récent (donc moderne), adapté au `tidyverse` (*e.g.*
    `dplyr`)
...
:::{.callout-note title="Instructions"}
Cartographiez le centre pondéré de la population des États-Unis depuis 1970 et
sa distance standard avec le package `ggplot2`.

Créez une nouvelle carte sur la base de l'ancienne. Il vous faudra "juste"
**changer certains jeux de données et nom de variables**.
...
`patchwork`. Utilisez la fonction `wrap_plots()` (*c.f.* @sec-comb-plots).
:::
...
Modifier la variable `year` (utilisez la fonction `plyr::round_any()`). Ensuite,
il vous faudra agréger les données.
:::
...
pond_disc <-
  mutate(center_pondered, year = as.factor(plyr::round_any(year, 10))) |>
  group_by(year) |>
...
Appuyez-vous sur les cartes précédentes. Ajoutez les lignes suivantes à votre
code `ggplot2` :
...
p <- ggplot() +
  geom_sf(data = states) +
  geom_point(data = pond_disc, aes(x = cw_x, y = cw_y), color = "red", size = 4) +
  geom_text(
    data = pond_disc,
  ...
  ggforce::geom_circle(
    data = pond_disc, # votre jeu de données non-pondéré
    aes(
      x0 = cw_x, # La moyenne des x
      y0 = cw_y, # La moyenne des y
      r = cw_sd  # la distance standard
...
#   width = 800,
#   height = 500,
#   renderer = gifski_renderer(glue::glue("{FIG_DIR}/weighted_center.gif"))
# )
