...
Dans le cas de **données massives** (volumineuse), on pourra se tourner vers le
package [`data.table`](https://r-datatable.com/) qui possède sa propre syntaxe.
Notez qu'on peut maintenant utilisez la syntaxe `dplyr` avec `data.table` en
utilisant [`dtplyr`](https://dtplyr.tidyverse.org/).
...
## Les matrices {#sec-struc-mat}

Comme un DataFrame, une matrice comporte des lignes et des colonnes. La
différence entre ces deux objets réside dans le fait qu'une matrice ne comporte
que des valeurs numériques.

Pour des manipulations plus avancées, rendez-vous à la @sec-manip-mat.
