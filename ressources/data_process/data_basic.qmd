...
Souvent, on souhaite sélectionner des données dans une gamme de valeurs, il
suffit de faire :
...
Pour convertir un format large (*wide*) en format long (*long*) on utilise la
fonction `tidyr::pivot_longer()`.
...
## Manipulation de matrice {#sec-manip-mat}
...
souvent utilisés pour optimiser des traitements. Avant de lire cette section,
renseignez-vous sur la structure de ce type d'objet (*c.f*. @sec-struc-mat).
...
sur les chaînes de caractères de guide-R (Larmarange, 2025)
...
renseigner [ici](https://egallic.fr/Enseignement/R/Book/regex.html).
